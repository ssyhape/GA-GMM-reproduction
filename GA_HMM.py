...
# 生成原始信号
def generate():
    ss = np.sin(2*np.pi*t/15)
    ss[:10] = np.random.normal(0, 0.1, 10)
    ss[40:] = np.random.normal(0, 0.1, 10)
    return ss
s1 = np.sin(2*np.pi*t/15)
# 添加高斯白噪声
s1[:10] = np.random.normal(0, 0.1, 10)
s1[40:] = np.random.normal(0, 0.1, 10)
s =s1
for i in range(3):# 4 = kelm
    tmp = generate()
    s = np.concatenate((s,tmp))
s = s.reshape(4,50)
pf = pd.DataFrame(s)
pf = pf.apply(LabelEncoder().fit_transform)
signal = pf.values
...
ans_loc =model.decode(np.atleast_2d(signal[0]).T)[1]
plt.plot(t,s1)
for i in range(len(ans_loc)):
    if ans_loc[i] == 1:
        plt.scatter(i, s1[i], color='red')
    else :
        plt.scatter(i,s1[i],color='blue')
plt.show()
random.randint()